...
The sub-pixel errors confirm that the theoretical Lucas--Kanade result agrees closely with the actual pixel locations.
...
The solution $\mathbf{h}$ that minimises $\|A\mathbf{h}\|$ subject to $\|\mathbf{h}\|=1$ is obtained via the Singular Value Decomposition $A = U\Sigma V^\top$: the last column of $V$ (corresponding to the smallest singular value) gives $\mathbf{h}$, which is reshaped into the $3\times3$ matrix $H$.

\subsubsection{RANSAC}

To handle outlier correspondences, RANSAC is used:
\begin{enumerate}
  \item Randomly sample 4 correspondences.
  \item Compute $H$ via DLT.
  \item Count inliers (reprojection error $<\varepsilon$).
  \item Repeat for $k$ iterations; keep the best $H$.
  \item Refit $H$ using all inliers.
\end{enumerate}

The required number of iterations for success probability $p$ with inlier ratio $w$:
\begin{equation}
  k = \frac{\log(1-p)}{\log(1-w^4)}.
\end{equation}

\subsubsection{Pose Recovery from Homography}

Given $H$ and $K$, compute $M = K^{-1}H = [\mathbf{m}_1\;\;\mathbf{m}_2\;\;\mathbf{m}_3]$. Then:
\begin{align}
  \mathbf{r}_1 &= \mathbf{m}_1 / \|\mathbf{m}_1\|, \label{eq:r1}\\
  \mathbf{r}_2 &= \mathbf{m}_2 / \|\mathbf{m}_1\|, \label{eq:r2}\\
  \mathbf{r}_3 &= \mathbf{r}_1 \times \mathbf{r}_2, \label{eq:r3}\\
  \mathbf{t} &= \mathbf{m}_3 / \|\mathbf{m}_1\|. \label{eq:t_recover}
\end{align}

Noise may cause $R = [\mathbf{r}_1\;\;\mathbf{r}_2\;\;\mathbf{r}_3]$ to violate orthogonality. It is projected onto $SO(3)$ via SVD:
\begin{equation}
  R_{\text{valid}} = U\,\mathrm{diag}(1, 1, \det(UV^\top))\,V^\top,
\end{equation}
where $U\Sigma V^\top = \mathrm{SVD}(R)$.
...
\subsubsection{Triangulation (Planar Simplification)}

For a planar object at $Z=0$, full triangulation is not necessary. Given the homography $H$ mapping world-plane points to image points:
\begin{equation}
  \begin{pmatrix}X \\ Y \\ 1\end{pmatrix} \sim H^{-1}\,\mathbf{x}.
\end{equation}
The 3D point is simply $(X, Y, 0)$.

\subsubsection{Reprojection Error}

The quality of the homography is measured by the mean reprojection error:
\begin{equation}
  E = \frac{1}{N}\sum_{i=1}^{N} \|\mathbf{x}_i - \hat{\mathbf{x}}_i\|_2,
\end{equation}
where $\hat{\mathbf{x}}_i = H\,\mathbf{x}'_i$ (dehomogenised).


% ------------------------------------------------------------------
\subsection{Feature Detection and Matching}

SIFT (Scale-Invariant Feature Transform) keypoints were detected in all four views and matched to the reference (front) view using brute-force matching with Lowe's ratio test (threshold 0.75).

\begin{table}[H]
\centering
\begin{tabular}{lcc}
\toprule
\textbf{View Pair} & \textbf{Good Matches} & \textbf{RANSAC Inliers}\\
\midrule
